...
func (s PostgresDB) CreateMessage(m model.Message) error {
	return gorm.G[model.Message](s.getDB()).Create(context.Background(), &m)
}

func (s PostgresDB) UpdateMessage(m model.Message) error {
...
func (s PostgresDB) DeleteMessage(m model.Message) error {
	_, err := gorm.G[model.Message](s.getDB()).Where("id = ?", m.ID).Delete(context.Background())
	return err
}

func (s PostgresDB) FindMessage(m model.Message) (model.Message, error) {
	message, err := gorm.
...
func (s PostgresDB) FindMessages(f model.MessageFilter) ([]model.Message, error) {
